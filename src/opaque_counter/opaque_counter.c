...
static void NO_COVERAGE test_opaque_counter_create(void) {
    ...
    assert(NULL == counter->history);
    assert(NULL == counter->name_label);
    assert(0 == counter->initial);
    assert(0 == counter->max);
